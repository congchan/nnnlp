...
from ..tf.bert import modeling
from ..tf.bert import optimization
from ..tf import tokenization
import tensorflow as tf
import json
import logging
from ..CoNLL2003_utils import CoNLL2013Processor
from ..util import (
    get_logger,
    to_json_string,
    reconstruct_from_estimator,
    SpanBasedF1Measure,
    write_eval
)
...
class Processor(CoNLL2013Processor):
...
def file_based_convert_examples_to_features(
        examples, label_map, max_seq_length, tokenizer, output_files):
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("decoder"):
        ...
        logging.info("num_labels: {}; logits: {}; labels:{}".format(
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits, input_mask):
                ...
                weighted_loss = tf.metrics.mean(values=per_example_loss,
                                                weights=tf.reduce_sum(input_mask,
                                                                      axis=-1) if FLAGS.use_crf else input_mask, )
...
def export_model(label_list):
  ...
  """
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        logging.info('write graph to file: %s' % pb_file)
...
def main(_):
    ...
    logging = get_logger(name='tensorflow', to_file=os.path.join(FLAGS.output_dir, 'log.log'))
    ...
    if FLAGS.do_train:
        ...
        logging.info("*** Writing training examples to tf record files ***")
        for train_file in train_files:
            logging.info("  %s", train_file)

        if not FLAGS.read_record:
            [tf.gfile.Remove(_f) for _f in tf.gfile.Glob(
                os.path.join(FLAGS.output_dir, "train.tf_record*"))]
            file_based_convert_examples_to_features(
                train_examples, label_map, FLAGS.max_seq_length, tokenizer, train_files)
    ...
    if FLAGS.do_eval:
        ...
        file_based_convert_examples_to_features(
            eval_examples, label_map, FLAGS.max_seq_length, tokenizer, eval_files)
        ...
        with open(output_eval_file, "a") as writer:
            logging.info(write_eval(writer, results, label_list, label_list[1:]))
        ...
        try:
            ...
            logging.info("ERROR: Output examples number not matched! \
        ...
        label = [example.labels for example in eval_examples]
        sb_measure = SpanBasedF1Measure()
        sb_measure(pred, label)
        logging.info(sb_measure.log_measure(output_eval_file))
